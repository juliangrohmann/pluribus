...
void test_uniform_translation(const Action a, const std::vector<Action>& actions, const PokerState& state, const Action mapped) {
  auto result = pseudo_harmonic_result(a, actions, state);
  REQUIRE(result.A == mapped);
  REQUIRE(result.B == mapped);
  REQUIRE(result.p_A == 1.0);
}

TEST_CASE("Pseudo harmonic action translation", "[actions]") {
  const std::vector actions = {Action::CHECK_CALL, Action{0.33}, Action{0.50}, Action{0.75}, Action::ALL_IN};
  const std::vector simple_actions = {Action{0.50}, Action{0.75}};
  PokerState state{3, {3'000, 3'000, 3'000}, 0, false};
  state = state.apply({Action{0.60}, Action::CHECK_CALL, Action::CHECK_CALL});
  test_uniform_translation(Action{0.50}, actions, state, Action{0.50});
  test_uniform_translation(Action::ALL_IN, actions, state, Action::ALL_IN);
  test_uniform_translation(Action{1.00}, simple_actions, state, Action{0.75});
  test_uniform_translation(Action{0.25}, simple_actions, state, Action{0.50});
  test_uniform_translation(Action{0.90}, actions, PokerState{2, 10'000, 0, false}, Action{0.75});
  constexpr float A = 0.50f, B = 0.80f, x = 0.60f;
  auto result = pseudo_harmonic_result(Action{x}, {Action{A}, Action{B}}, state);
  double p_A = (B - x) * (1 + A) / ((B - A) * (1 + x));
  REQUIRE(abs(result.p_A - p_A) < 0.01);
  REQUIRE(result.A == Action{A});
  REQUIRE(result.B == Action{B});
}
