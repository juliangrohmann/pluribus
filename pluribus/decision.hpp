...
public:
  ...
  float frequency(const Action a, const PokerState& state, const Board& board, const Hand& hand) const override {
    ...
    const auto a_it = std::ranges::find(actions, a);
...
public:
  Action next_action(CachedIndexer& indexer, const PokerState& state, const std::vector<Hand>& hands, const Board& board, const LosslessBlueprint* bp) const override {
    ...
    const hand_index_t hand_idx = indexer.index(board, hands[state.get_active()], static_cast<int>(state.get_round()));
    const int cluster = FlatClusterMap::get_instance()->cluster(state.get_round(), hand_idx);
    const size_t base_idx = bp->get_strategy().index(state, cluster);
    const auto freq = calculate_strategy(&bp->get_strategy()[base_idx], actions.size());
