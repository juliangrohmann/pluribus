...
public:
  ...
  explicit SamplingAlgorithm(uint64_t init_mask = 0L);
  explicit SamplingAlgorithm(const std::vector<uint8_t>& dead_cards);
...
public:
  explicit ImportanceSampler(const std::vector<PokerRange>& ranges, const std::vector<uint8_t>& dead_cards = {});
...
public:
  explicit ImportanceRejectionSampler(const std::vector<PokerRange>& ranges, const std::vector<uint8_t>& dead_cards = {});
...
private:
  std::vector<omp::FastUniformIntDistribution<>> _uniform_dists;
};

class ImportanceRandomWalkSampler : public ImportanceRejectionSampler {
public:
  explicit ImportanceRandomWalkSampler(const std::vector<PokerRange>& ranges, const std::vector<uint8_t>& dead_cards = {});
...
class RoundSampler {
public:
  explicit RoundSampler(const std::vector<PokerRange>& ranges, const std::vector<uint8_t>& dead_cards = {});

  RoundSample sample();
  void next_sample(RoundSample& sample);
  void set_mode(const SamplingMode mode) { _mode = mode; }
