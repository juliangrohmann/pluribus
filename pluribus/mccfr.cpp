...
int has_player_vpip(const PokerState& state, const PokerState& init_state, int p_idx) {
...
std::string strategy_label(const PokerState& state, const PokerState& init_state, Action action, bool phi) {
  auto rel_actions = state.get_action_history().slice(init_state.get_action_history().size()).get_history();
